<template>
  <el-container style="opacity: 0.9; width: 75%; margin: 35px auto 0 auto" class="message">
    ...
    <el-card v-if="dataList.comments.length>0" class="animate__animated animate__fadeInLeft">
      <ul style="padding: 0;" class="comment-list">
        <li class="comment" v-for="comment in dataList.comments" :key="comment.id">
          <el-avatar :src="comment.user.blogFile.publishUrl" @error="avatarLoadError">
            <el-avatar icon="el-icon-user-solid" style="background-color: #666"></el-avatar>
          </el-avatar>
          <div class="content">
